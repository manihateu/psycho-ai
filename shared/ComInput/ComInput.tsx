...
const ComInput = ({placeholder, classNames, isPassword, isAccepted, value, onChange, onSubmit} : TComInputProps) => {
    const [visible, setVisible] = useState <boolean> (true)
    return (
    <View className={`relative ${classNames}`}>
        <TextInput onSubmitEditing={onSubmit} onChangeText={onChange} secureTextEntry={isPassword ? visible : false} value={value} placeholder={placeholder} className='bg-[#F2F3F7] p-[20px] rounded-2xl font-Comfortaa'/>   
